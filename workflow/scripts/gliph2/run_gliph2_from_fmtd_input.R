...
option.list <- list(
    # Server to work on.
    make_option(opt_str="--ToolPath", type="character", default=NULL, dest="tool.path", help="Char, absolute path to tool source code.\n"),
    make_option(opt_str="--RefPath", type="character", default=NULL, dest="ref.path", help="Char, absolute path to tool-specific reference sets directory.\n"),
...
tool.path <- opt$tool.path
ref.path <- opt$ref.path
...
tmp.path <- paste0(tmp.path, '/', tmp.user)
